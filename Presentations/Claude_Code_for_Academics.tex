...
\subtitle{An AI-Powered Agent for Empirical Research}
\author{Alessandro Spina\thanks{This talk is in the spirit of Robin Hood: everything is stolen with the intent to help the reader.}}
...
\begin{column}{0.47\textwidth}
  \begin{tcolorbox}[
    ...
    A dedicated (team of) "RAs" who reads your data, runs code, builds your slides, and \emph{reasons with you}.
  \end{tcolorbox}
\end{column}
\end{columns}
...
\vspace{0.05cm}
\begin{quotebox}
  \textit{The real shift happens when you stop treating Claude as a tool that \emphcoral{executes instructions} and start treating it as a colleague who \emphgold{thinks alongside you}.}
\end{quotebox}
\end{frame}



\begin{frame}{Today's Objective: Tempt You to the Dark Side\ldots}
\vspace{0.1cm}
\begin{center}
...
\begin{frame}{Setup Claude Code - via terminal}
\vspace{0.05cm}
\begin{columns}[T]
\begin{column}{0.42\textwidth}
  \begin{codeblock}
    \codett{\textcolor{warmgold}{\$} npm install -g @anthropic-ai/}\\
    \codett{\phantom{\textcolor{warmgold}{\$} }claude-code}\\[3pt]
    \codett{\textcolor{warmgold}{\$} cd \textasciitilde/research/asset\_pricing}\\[3pt]
    \codett{\textcolor{warmgold}{\$} claude}\\[6pt]
    \codett{\textcolor{warmgold}{>} Read the data in data/raw/}\\
    \codett{\quad crsp\_monthly.csv, run}\\
    \codett{\quad summary statistics, and}\\
    \codett{\quad create a figure showing the}\\
    \codett{\quad cross-section of returns}\\
   \end{codeblock}
\end{column}
\begin{column}{0.42\textwidth}
  \vspace{0.3cm}
  {\small Three commands to install and launch. Then you simply describe what you want in plain English.}

  \centering
  \includegraphics[width=\linewidth]{figures/Screenshot 2026-02-27 134521.png}
  \vspace{0.3cm}
  {\small "Codebase" = Your folder}
...
\begin{frame}{Alternative: Desktop Claude Code}
\vspace{-0.05cm}
...
\begin{column}{0.52\textwidth}
  \begin{tikzpicture}[
    ...
    \node[plan, below=of p1] (p2) {\textbf{\color{navy}Max (\$100--200/mo)} --- 5--20$\times$ more usage. Needed for serious research work.};
    \node[plan, below=of p2] (p3) {\textbf{\color{navy}API (pay-per-token)} --- most flexible but requires technical setup. Cost varies with usage.};
  \end{tikzpicture}
\end{column}
\begin{column}{0.45\textwidth}
  \begin{keyinsight}[Is It Worth It?]
    ...
    {\scriptsize\color{slate} Pricing changes frequently --- check \url{claude.ai/pricing}.}}
  \end{keyinsight}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Why Claude Code? The Alternatives}
\vspace{0.05cm}
\begin{tikzpicture}[
...
\begin{frame}{Now What? Things You Can Ask Claude to Do}
\vspace{0.1cm}
...
%===============================================================================
\section{Understanding Claude Code}
%===============================================================================
...
\begin{frame}{Example: /The Editor}

{\small The Editor persona is a structured markdown file that tells Claude exactly how to audit your paper's prose, structure, and argumentation. Something like: \url{https://www.refine.ink/}}
\vspace{0.05cm}
\begin{center}
...
\begin{center}
  \includegraphics[width=0.4\linewidth]{figures/you-get-an-ai-agent.jpg}
\end{center}
\end{frame}
...
\draw[arrow] (orch) -- (a3);
\draw[arrow] (orch) -- (a4);
\draw[arrow, darkblue!40] (a1.south) -- ++(0,-0.35) -| (orch.south);
\draw[arrow, darkblue!40, dashed] (a4.south) -- ++(0,-0.35) -| (orch.south);
\node[font=\scriptsize, text=slate] at (0, -2.5) {Each agent runs in parallel --- results merged by the orchestrator};
\end{tikzpicture}
\vspace{0.05cm}
\begin{keyinsight}[Subagents vs.\ Agent Teams]
  {\small \emphgold{Subagents} handle one task sequentially. \emphgold{Agent Teams} run \emph{multiple specialists in parallel}, then the orchestrator merges results and enforces quality gates.}
\end{keyinsight}
...
%===============================================================================
\section{The workflow: How to use Claude}
%===============================================================================

% ---------- SLIDE: So What Can We Do ----------------------------------------
\begin{frame}{So What Can We Do with Claude Code?}
\vspace{0.1cm}
\begin{center}
\begin{tikzpicture}[
  capbox/.style={fill=navy, text=white, rounded corners=3pt,
  ...
  \node[capbox] (c1) at (0, 0) {Write Code};
  \node[capbox] (c2) at (4.0, 0) {Run Analysis};
  \node[capbox] (c3) at (8.0, 0) {Search the Web};
  \node[capbox] (c4) at (0, -1.2) {Reorganise Folders};
  \node[capbox] (c5) at (4.0, -1.2) {Prepare Slides};
  \node[capbox] (c6) at (8.0, -1.2) {Review Code/Papers};
\end{tikzpicture}
\end{center}
...
\begin{column}{0.52\textwidth}
  ...
  \vspace{0.15cm}
  \begin{keyinsight}
    This entire deck was built and styled with Claude Code. Every TikZ diagram, every tcolorbox, every slide layout.
  \end{keyinsight}
\end{column}
\begin{column}{0.45\textwidth}
  \begin{quotebox}
...
% ---------- SLIDE: Checking Code --------------------------------------------
\begin{frame}{Checking Code}
\begin{columns}[T]
\begin{column}{0.6\textwidth}
  \begin{keyinsight}[The Cunningham Conjecture]
...
\begin{frame}{Tier 2: The Working System}
...
\begin{frame}{Tier 3: The Full Setup}
...
\begin{quotebox}
  {\small Most academics will get \emphgold{most value} from Tier 1 alone. A good CLAUDE.md and a reliable progress log system is \emphgold{transformative} compared to having nothing.}
\end{quotebox}
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (t1) -- (t2);
  \draw[arrow] (t2) -- (t3);
  \node[font=\tiny, text=slate, below=0.1cm of t1] {CLAUDE.md + 2 commands};
...
\begin{frame}{AI Hallucinations Are Real --- But Manageable}
\begin{columns}[T]
\begin{column}{0.47\textwidth}
  \begin{tcolorbox}[
    enhanced, colback=red!5, colframe=red!30, boxrule=0.5pt,
...
% ---------- SLIDE: Academic Integrity & Disclosure --------------------------
\begin{frame}{Academic Integrity \& Disclosure}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
  \begin{warningbox}[The Open Questions]
    {\small
    \emphgold{1.} Should you disclose AI use in papers?\\[3pt]
    \emphgold{2.} What do journals say?\\[3pt]
    \emphgold{3.} Can an LLM be a co-author?\\[3pt]
    \emphgold{4.} What about reproducibility when outputs are non-deterministic?}
  \end{warningbox}
\end{column}
\begin{column}{0.47\textwidth}
  \begin{keyinsight}[Emerging Norms]
    {\small Most top journals now \emphgold{require disclosure} of AI tool use (AER, RFS, JF policy statements).\\[4pt]
    Best practice: acknowledge in a footnote, describe what the tool did, and ensure all results are \emphgold{independently verifiable}.\\[4pt]
    The CLAUDE.md + session logs create a natural \emphsage{audit trail} of what Claude did.}
  \end{keyinsight}
\end{column}
\end{columns}
\end{frame}
...
% ---------- SLIDE: Reproducibility & Non-Determinism -----------------------
\begin{frame}{Reproducibility \& Non-Determinism}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
  \begin{warningbox}[The Problem]
    {\small Claude does \emphcoral{not} produce identical output for identical prompts. If your research pipeline includes an AI step, can someone else replicate it?}
  \end{warningbox}
  \vspace{0.08cm}
  {\small\bfseries\color{navy} The mitigation:}
  \begin{itemize}\scriptsize
    \item Use Claude to \emphgold{write code}, not to produce results directly
    \item The code itself is deterministic and replicable
    \item Cross-language verification catches errors regardless
    \item Session logs document what was done
  \end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
  \begin{keyinsight}[The Principle]
    {\small Claude is the \emphgold{cook}, not the \emphgold{recipe}. The recipe (your code) is what gets published and replicated. How it was written matters less than whether it runs correctly.}
  \end{keyinsight}
  \vspace{0.1cm}
  {\small\color{slate} Also: beware \emphcoral{over-reliance}. If you can't understand the code Claude wrote, you can't defend it in a seminar. The researcher remains the final authority.}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Can AI Write a Paper?}
{\small The \emphgold{Automated Paper Engine} (APE) project tests whether AI agents can autonomously produce economics research --- from idea to finished paper. \url{https://ape.socialcatalystlab.org/}}
\begin{center}
  \includegraphics[width=0.55\linewidth]{figures/Screenshot 2026-03-03 151111.png}
\end{center}
\begin{center}
...
\begin{tikzpicture}[
  ...
  \node[point, below=of p1] (p2) {\hspace{0.45cm}Startup costs (and actual \$ costs) are non-trivial. The real benefits take time.};
  \node[num] at ([xshift=-0.25cm]p2.west) {2};
  \node[point, below=of p2] (p3) {\hspace{0.45cm} There are no solutions, only tradeoffs: Beware of complexity.  };
  \node[num] at ([xshift=-0.25cm]p3.west) {3};
\end{tikzpicture}
\begin{center}
     \includegraphics[width=0.45\linewidth]{figures/oh-agent-smith.png}
\end{center}
\end{frame}
...
\vfill
\begin{center}
  {\Large\bfseries\textcolor{darkblue}{Thank you.}}\\[12pt]
\end{center}
\vfill
\end{frame}
}
...
\begin{column}{0.50\textwidth}
  \begin{warningbox}[The Problem]
    {\small Every new project needs the same boilerplate: folders, CLAUDE.md, README.md, directory structure. Doing this manually is tedious and error-prone.}
  \end{warningbox}

  \vspace{0.1cm}
  \begin{keyinsight}[The Solution]
    {\small \textbf{setup\_project.ps1} --- a PowerShell script that runs in your Windows terminal \emphgold{before Claude Code is involved}.\\[4pt]
    It creates folders and copies template files. Nothing to do with Claude --- just automating file system operations.}
  \end{keyinsight}
\end{column}
\begin{column}{0.47\textwidth}
  \begin{codeblock}
