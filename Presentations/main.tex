...
\begin{tikzpicture}[
  ...
  \node[cat] (c1) {\textbf{\color{navy}Agentic} --- reads/edits files, runs code, uses tools (MCP), with persistent memory/instructions via markdown files.};
  ...
  \node[tool, below=of t1] (t2) {\quad\textbf{\color{navy}OpenAI Codex CLI} --- terminal-based, similar model to Claude Code. Newer, less mature ecosystem.};
  \node[tool, below=of t2] (t3) {\quad\textbf{\color{navy}GitHub Copilot} --- Plugs in to existing IDEs (e.g. VS Code). Code, preview, and AI chat in one window. Also accessible via github.com.};
  \node[tool, below=of t3] (t4) {\quad\textbf{\color{navy}Cursor} --- standalone AI-native IDE based on VS Code.};
  \node[cat, below=of t4] (c2) {\textbf{\color{navy}Non-agentic} --- \textbf{\color{navy}ChatGPT / Gemini}: browser-based chat. Cannot operate on your local project files or maintain context across sessions.};
\end{tikzpicture}
