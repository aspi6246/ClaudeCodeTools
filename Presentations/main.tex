...
\begin{frame}{What Is a CLAUDE.md file?}
  \begin{keyinsight}[How It Works]
    ...
    Claude writes in markdown (plain text). LLMs have been trained to read and write markdown easily. Great, if you want to switch model. Just point Codex/Gemini/Llama/whatever at the markdown files in your folder.
  \end{keyinsight}
\end{frame}
...
\begin{column}{0.55\textwidth}
  ...
  \begin{itemize}\small
    \item Keep CLAUDE.md concise --- Claude follows \emphcoral{\textasciitilde100--150 lines} reliably. Detailed rules go in separate files.
...
\end{column}
\begin{column}{0.48\textwidth}
  {\small Build your CLAUDE.md, READEME.md, Session logs. These files live in your project root. When you start a new Claude Code session, it reads this automatically and immediately knows:}
  \vspace{0.2cm}
  \begin{itemize}\small
...
\begin{tikzpicture}[
  ...
  \node[modebox, fill=red!8, draw=red!40, below=of m3] (m4) {%
    \hspace{0.6cm}\textbf{YOLO mode} \hfill {\small\color{coral} Bypasses \emphcoral{all} checks. Full autonomy.} \hspace{0.1cm}};
...
\begin{column}{0.52\textwidth}
  \begin{keyinsight}[What Is a Persona?]
    {\small A \emphgold{persona} is a markdown file that defines a \emph{role} --- tone, expertise, audit checklists, and structured deliverables.\\[6pt]
        e.g.
“Teaching assistant persona” (pedagogical, beginner-friendly)

“Copyeditor persona” (grammar and clarity above all)
    .}
  \end{keyinsight}
\end{column}
