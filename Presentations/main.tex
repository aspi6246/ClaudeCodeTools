...
\begin{tikzpicture}[
  ...
  cat/.style={fill=navy!8, draw=navy!30, rounded corners=3pt,
    text width=12cm, inner sep=5pt, font=\small, align=left},
  node distance=0.06cm,
]
  \node[cat] (c1) {\textbf{\color{navy}Agentic systems} read/edit files, run code, use tools (MCP). Persistent memory/instructions via markdown files (so no vendor lock in: if you switch tools, your project knowledge transfers).};
  \node[tool, below=of c1] (t1) {\quad\textbf{\color{navy}Claude Code} --- terminal-based. No IDE needed --- works alongside your existing tools (e.g. RStudio, Stata, Overleaf).};
  \node[tool, below=of t1] (t2) {\quad\textbf{\color{navy}OpenAI Codex CLI} --- terminal-based. Newer, less mature ecosystem.};
  \node[tool, below=of t2] (t3) {\quad\textbf{\color{navy}GitHub Copilot} --- Use in existing IDEs. Code, preview, & AI chat in one window. Also accessible via github.com.};
  \node[tool, below=of t3] (t4) {\quad\textbf{\color{navy}Cursor} --- standalone AI-native IDE based on VS Code.};
  \node[cat, below=of t4] (c2) {\textbf{\color{navy}Non-agentic systems: ChatGPT / Gemini} --- browser-based chat. Cannot operate on your local project files or maintain context across sessions.};
\end{tikzpicture}
\end{frame}
...
\begin{column}{0.47\textwidth}
  \begin{tcolorbox}[
    ...
    {\small\bfseries\color{coral} Before:}\\[4pt]
...
\begin{column}{0.52\textwidth}
  \vspace{0.1cm}
  \begin{quotebox}
    {\small The academic culture wars around AI have begun. Some will shake their heads at the outsourcing of slide decks to Claude.}
  \end{quotebox}

  \vspace{0.1cm}
  \begin{keyinsight}
    {\small My take: Nobody's tombstone reads \emphgold{``Wrote his own \LaTeX{} code.''} \\[4pt]
...
\begin{column}{0.48\textwidth}
  \begin{keyinsight}[Why This Matters]
    Aim to keep your CLAUDE.md file to \emphgold{100--150 lines}. When you accumulate too many rules, path-scoping keeps the active instruction set small and relevant, avoiding \emphcoral{information overload} that degrades Claude's performance.
  \end{keyinsight}
\end{column}
\end{columns}
\end{frame}
